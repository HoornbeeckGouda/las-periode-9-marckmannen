<?php

namespace App\Http\Controllers;

use App\Models\CourseYear;
use App\Http\Requests\StoreCourseYearRequest;
use App\Http\Requests\UpdateCourseYearRequest;

class CourseYearController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index()
    {
        //
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create()
    {
        //
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(StoreCourseYearRequest $request)
    {
        //
    }

    /**
     * Display the specified resource.
     */
    public function show(CourseYear $courseYear)
    {
        //
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(CourseYear $courseYear)
    {
        //
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(UpdateCourseYearRequest $request, CourseYear $courseYear)
    {
        //
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(CourseYear $courseYear)
    {
        //
    }
}
